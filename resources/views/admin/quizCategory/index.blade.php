...
@section('content')
        ...
        <div class="content">
            ...
            <div class="card card-default faq">
                ...
                {{-- <div class="card-head mb-2">
                    @if (!empty($pre) && $pre->is_modify == 'yes')<a href="{{route('quizCategory.create')}}" class="btn btn-pill btn-primary">Add Category</a>
                    @endif
                </div> --}}
                        ...
                        <thead>
                            <tr>
                                ...
                                @if (!empty($pre) && $pre->is_modify == 'yes')
                                    <th scope="col" style="width: 150px;">Action(s)</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody>
    ...
    <script type="text/javascript">
        ...
        $("body").on('click', '.delete', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "delete",
                        url: '{{ url('quiz-delete-category') }}' + "/" + id,
                        headers: {
        ...
        $(document).on('change', '.priority-input', function() {
            ...
            $.ajax({
                url: "{{ route('quiz-category.updatePriority') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: quizId,
